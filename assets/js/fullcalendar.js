...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a name="myDivID" class="' + classes.join(' ') + '"' +
			(event.url ?
				' href="' + htmlEscape(event.url) + '"' :
				''
				) +
			...
			'>' +
				'<div class="fc-content">' +
					(timeText ?
						...
						'</div>'+
          '<input class="ronny_ko" name="ronny_ok" id="ronny_kk" type="hidden" value="'+htmlEscape(fullTimeText)+'">':
						''
						) +
					(event.title ?
						'<div class="fc-title">' +
	...
	function renderSection(position) {
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
              ...
              var html_ronny='<button "type="button" class="' + classes.join(' ') + '">' +
              innerHtml +
              '</button>';
if(buttonName=='month')
{
  html_ronny='<button "type="button" onclick="donnee_mois()" class="' + classes.join(' ') + '">' +
    innerHtml +
    '</button>'
}else{
  if(buttonName=='agendaDay'){
    html_ronny='<button "type="button" onclick="donnee_jour()" class="' + classes.join(' ') + '">' +
      innerHtml +
      '</button>'
  }else{
    if(buttonName=='agendaWeek'){
      html_ronny='<button "type="button" onclick="donnee_semaine()" class="' + classes.join(' ') + '">' +
        innerHtml +
        '</button>'
    }else{
    //  alert('planning not passed')
    }
  }
}
